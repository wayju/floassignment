...
describe('testing NEM12Record', () => {
  test('test basic NEM12 record parsing', () => {
    const line = csvToString('100,NEM12,200506081149,UNITEDDP,NEMMCO');
    const record = NEM12Record.parse(line, timeZone);
...
describe('testing NEM12_100Record', () => {
  test('test basic NEM12 record parsing', () => {
    ...
    const record100 = record as NEM12_100Record;
...
describe('testing NEM12_200Record', () => {
  test('test basic NEM12 record parsing', () => {
    ...
    const record200 = record as NEM12_200Record;
...
describe('testing NEM12_300Record', () => {
  test('test basic NEM12 record parsing', () => {
    ...
    const record200 = record as NEM12_300Record;

    expect(record200.getIntervalDate()).toEqual(
      new Date('2005-03-04T00:00:00+11:00')
    );
    expect(record200.getQualityMethod()).toBe('A');
    expect(record200.getReasonCode()).toBe('');
    expect(record200.getReasonDescription()).toBe('');
    expect(record200.getIntervalValues()[0]).toEqual(0);
    expect(record200.getIntervalValues()[47]).toEqual(0.612);
    expect(record200.getUpdateDateTime()).toEqual(
      new Date('2005-03-10T12:10:04+11:00')
    ); //DST
    expect(record200.getMSATSLoadDateTime()).toEqual(
      new Date('2005-03-10T18:22:04+11:00')
    );
  });
});
